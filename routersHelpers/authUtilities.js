...
const tokenGenerator = async (user, res) => {
    ...
    const accessToken = await jwt.sign( { AccountNumber, Role}, process.env.JWT_ACCESS_TOKEN, {expiresIn: '9999d'} );
    const refreshToken = await jwt.sign( { AccountNumber, Role}, process.env.JWT_REFRESH_TOKEN, {expiresIn: '9999d'} );
